...
def fix_pre_commit_hook(repo_path):
    hook_file_path = os.path.join(repo_path, '.git', 'hooks', 'pre-commit')
    if os.path.exists(hook_file_path):
        with open(hook_file_path, 'rb') as file:
            content = file.read()
        # Remover caracteres \r e garantir \n como terminadores de linha
        fixed_content = content.replace(b'\r\n', b'\n').replace(b'\r', b'\n')
        with open(hook_file_path, 'wb') as file:
            file.write(fixed_content)
        print(f'Corrigido o arquivo de hook: {hook_file_path}')
...
def auto_commit(repo_path):
    ...
    fix_pre_commit_hook(repo_path)  # Corrigir terminadores de linha no hook pre-commit
